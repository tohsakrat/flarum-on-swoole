...
## 压测
关掉全部缓存机制，完整鉴权和序列化的纯动态请求。主机是gb5单核450分的netlab洋垃圾。

### co模式
<img width="1103" height="169" alt="image" src="https://github.com/user-attachments/assets/81010c21-16f8-43a6-a046-e097c18240bd" />
<img width="3624" height="430" alt="image" src="https://github.com/user-attachments/assets/dae42a1b-cb9f-4192-8db0-cf50ad1a04bf" />
<img width="1224" height="435" alt="image" src="https://github.com/user-attachments/assets/239c8183-34ef-4d71-9ecb-9dc88a499c09" />



co版本对序列化过程中n+1查询问题可能能多个数百毫秒的提升（合并部分可以合并的请求），空空闲基本可以跑到峰值的300ms响应时间。但是如果自己照着日志把tags、fof folloers，fof badgegs，fof poll等插件的n+1查询改了，就几乎没有提升了。而co模式相比woker模式本身有隔离状态和各种反射的额外开销，所以具体用哪种模式还要根据实际情况。
