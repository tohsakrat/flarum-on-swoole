...
3. Serializer并行化，合并其中的n+1查询语句（需要co模式）。启动后需要运行至少两次，学习哪些序列化需要被并行化，哪些不需要，以达到最佳性能。
...
注意：
- 如果改了设置，不论额外css、header、footer、插件设置、用户组权限（授予用户不受影响）都需要完整重启一次，否则配置不生效，还可能会出现未知问题。
- 因为不同论坛插件安装情况不同，补丁不能改覆盖到所有插件，co模式大概率无法完全开箱即用。遇到报错可以参照已经有的插件补丁，继续打适配补丁。
...
对于flarum最后也是最麻烦的性能问题，cpu计算过于密集，而且因为这东西渲染帖子正文都是要注入权限组，和其他任何插件作者想的数据模型的甚至可以临时查表的，要缓存的话策略得根据自己的插件组合来定制，所以并没有包含在公开库内，cpu纯计算阻塞会严重影响主机资源利用率，能做的有限。



```
题外话，觉得flarum本身“很轻”，闲时也能让用户等个一两秒也没关系的别用哈。管一个动辄把cpu占满一两秒的东西叫轻量也是难绷。
类似的思路两年前就有人搞出来了，人家在群里提了句就被你们追着喷，一项并不算特别难的优化一直没人肯分享真是活78该。
```
