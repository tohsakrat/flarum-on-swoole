...
## 压测
关掉全部缓存机制，完整鉴权和序列化的纯动态请求

### co模式
<img width="1103" height="169" alt="image" src="https://github.com/user-attachments/assets/81010c21-16f8-43a6-a046-e097c18240bd" />
<img width="3612" height="289" alt="image" src="https://github.com/user-attachments/assets/66eec142-24e4-4c97-8deb-e4f46b102005" />


co版本对序列化过程中n+1查询问题严重的插件可能能多个数百毫秒的提升，空空闲基本可以跑到峰值的300ms响应时间。但是如果自己照着日志把tags、fof folloers，fof badgegs，fof poll等插件的n+1查询改了，就几乎没有提升了。而co模式相比woker模式本身有隔离状态和各种反射的额外开销，所以具体用哪种模式还要根据实际情况
