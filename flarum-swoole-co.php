...
class CoroutineDbProxy implements \Illuminate\Database\ConnectionResolverInterface
{
    ...
    {
        $this->getManager()->setDefaultConnection($name);
    }
    public function __call($method, $args)
    {
        return $this->getManager()->$method(...$args);
    }
}
...
$server->on('workerStart', function (Server $server, int $workerId) {
    ...
    if ($redisManager) {
        try {
            ...
            if (empty($rawCfgs)) {
                echo "[Worker #{$workerId}] ⚠️ Redis rawConfigs 提取失败，降级为原始 manager。\n";
            } else {
    ...
    // 清除由于启动时可能已经实例化的依赖组件，强制它们使用新的代理
...
/**
 * 将 Swoole Request 转换为 PSR-7 Request
 */
...
/**
 * 发送 PSR-7 Response 到 Swoole Response
 */
