...
$server->on('request', function (SwooleRequest $swooleReq, SwooleResponse $swooleRes) use ($server) {
    ...
    if ($isDoc) {
        ...
        if (!empty($assets)) {
            ...
            // 注意：在 Swoole 非协程的异步 Worker 模式下，直接调用 $server->send() 
            // 往 HTTP FD 写入原始 103 响应，会与 Reactor 线程的 HTTP 状态机产生 IPC 竞争，
            // 导致 HTTP 报文严重损坏（白屏或 502）。因此这里去掉了 $server->send，
            // 仅保留最下方 200 OK 时的 Link 头注入，依然能实现 Preload 效果！
        }
    }
...
function applyCdnRewrite(string $content): string
{
    ...
    if ($debug) {
        error_log('[CDN-Rewrite] Enter. len=' . strlen($content) . ' has_assets=' . (str_contains($content, '/assets/') ? 'yes' : 'no'));
    }
